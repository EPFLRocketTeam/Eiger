    ...
    def __init__(self):
        ...
        if sys.platform.startswith('win'):
            ...
            if self.__m_dllBasic == None:
                print ("Exception: The PCANBasic DLL couldn't be loaded!")
        elif sys.platform.startswith('linux'):
            self.__m_dllBasic = cdll.LoadLibrary('libpcanbasic.so')
            if self.__m_dllBasic == None:
                print ("Exception: The PCAN-Basic (libpcanbasic.so) couldn't be loaded!")
        elif sys.platform == 'darwin':
            self.__m_dllBasic = cdll.LoadLibrary('libPCBUSB.dylib')
            # print "hi"
            if self.__m_dllBasic == None:
                print ("Exception: The PCAN-Basic (libPCBUSB.dylib) couldn't be loaded!")
        else:
            sys.exit("Error: OS platfrom not supported!")
